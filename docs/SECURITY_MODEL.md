...
- `iCloudDrive` (the default) puts anchors in iCloud Drive so they leave the Mac through file sync. Unsandboxed builds resolve `~/Library/Mobile Documents/com~apple~CloudDocs/MacAlarm`. The sandboxed Mac App Store build resolves the app's iCloud **ubiquity container** — `url(forUbiquityContainerIdentifier: "iCloud.com.jctec.macalarm")/Documents/MacAlarm` — which appears in iCloud Drive as `~/Library/Mobile Documents/iCloud~com~jctec~macalarm/Documents/MacAlarm`.
